...
from view import AddNoteWindow, Geometry
...
def main_window():
    ...
    button_show = tk.Button(text='Показать заметки', command=Controller.show_notes)
    ...
    button_save_file = tk.Button(text='Сохранить файл', command=Controller.save_notes)
    button_save_file.grid(column=0, row=3, stick='ew')
    button_add_contact = tk.Button(text='Добавить заметку', command=AddNoteWindow.open_window_add)
    button_add_contact.grid(column=0, row=4, stick='ew')
    button_change_contact = tk.Button(text='Изменить заметку', command=Controller.change_note)
    button_change_contact.grid(column=0, row=5, stick='ew')
    button_delete_contact = tk.Button(text='Удалить заметку', width=20, command=Controller.delete_note)
    button_delete_contact.grid(column=0, row=6, stick='ew')
    lable = tk.Label(text='Для поиска введите заметку:')
    lable.place(x=2, y=180)

    def search_contact(event):
        search_text = entry.get()
        Controller.search_note(search_text)
    ...
    stl = ttk.Style()
    stl.configure('Treeview', rowheight=60)
    heads = ['id', 'Наименование', 'Дата', 'Заметка']
